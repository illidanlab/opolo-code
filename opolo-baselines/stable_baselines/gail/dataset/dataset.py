...
class ExpertDataset(object):
    ...
    def __init__(self, expert_path=None, traj_data=None, train_fraction=0.7, batch_size=64,
        ...
        else: # saving memory for atari games 
            next_observations = np.concatenate([traj_data['obs'][1:traj_limit_idx],traj_data['last_observations'][:]])
        ...
        self.demo_dones = np.concatenate((episode_starts[1:], np.array([1]))) # array of indiciator, where 1 means the end of episode -- Judy

        self.returns = episode_returns 
    ...
    def get_transitions(self, n=None):
        """
        get a group of state, action pairs
        """
        #tmp_indices = np.random.permutation(self.num_transition.astype(np.int64))
        #if n is not None and n < self.num_transition:
        #    tmp_indices = tmp_indices[:n]
        #actions = self.actions[tmp_indices]
        #observations = self.observations[tmp_indices]
        #return observations, actions 
        return self.observations, self.actions, self.rewards, self.demo_dones, self.next_observations, self.returns
    
    def add_absorbing_states(self, data, env):
        ...
        """
        states, actions, rewards, dones, next_states, returns = data
        # First add 0 indicator to all non-absorbing states.
        states = np.pad(states, ((0, 0), (0, 1)), mode='constant')
        next_states = np.pad(
            next_states, ((0, 0), (0, 1)), mode='constant')

        states = [x for x in states]
        next_states = [x for x in next_states]
        actions = [x for x in actions]
        dones = [float(x) for x in dones] 
        ...
        i = 0
        current_len = 0
        print(states[0].shape, env.envs[0].get_absorbing_state().shape)
        while i < len(states):
            current_len += 1
            # if this episode is non-truncate-terminated -- Judy
            if dones[i] and current_len < env.envs[0].max_episode_steps:  # pylint: disable=protected-access
                current_len = 0
                states.insert(i + 1, env.envs[0].get_absorbing_state())
                next_states[i] = env.envs[0].get_absorbing_state()
                next_states.insert(i + 1, env.envs[0].get_absorbing_state())
                actions.insert(i + 1, np.zeros((env.action_space.shape[0],)))
                dones[i] = 0.0             # change this state to un-done, followed by several absorbing states. -- Judy
                rewards.insert(i+1, 0.0)    # we will replace this reward with intrinsic reward later. -- Judy
        ...
        return states, actions, rewards, dones, next_states, returns
    ...
    def log_info(self):
        ...
        logger.log("Sample rate: {}".format(self.sample_rate))
    ...
    def get_next_batch(self, split=None):
        ...
        if dataloader is None:
            print("Warning !  dataloader is None")
            time.sleep(3)
            self.init_dataloader(self.batch_size)
            dataloader = {
                None: self.dataloader,
                'train': self.train_loader,
                'val': self.val_loader
