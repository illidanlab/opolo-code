...
class GAIfODiscriminator(object):
    def __init__(self, observation_space, hidden_size=256,
        ...
        p = tf.clip_by_value(1 - tf.nn.sigmoid(generator_logits), 1e-4, 1 - 1e-8)
    ...
    def train_onpolicy_discriminator(self, writer, logger, d_gradient_steps, d_learning_rate, batch_size, teacher_buffer, seg, num_timesteps, sess, d_adam, nworkers):
        ...
        for i in range(d_gradient_steps):
            ...
            ob_expert, next_ob_expert, ob_batch, next_ob_batch = self.generate_onpolicy_data(teacher_buffer, observations, next_observations, batch_size, sess)
