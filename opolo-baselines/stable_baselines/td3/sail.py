...
from stable_baselines.deepq.replay_buffer import ReplayBuffer, TrajectoryBuffer 
...
from stable_baselines.gail.curisosity import TransitionCuriosityClassifier 
from stable_baselines.gail.dataset.dataset import ExpertDataset


class SAIL(OffPolicyRLModel):
    """
    Learning from Observation based on Twin Delayed DDPG (TD3)
    ...
    def __init__(self, policy, env, gamma=0.99, learning_rate=3e-4, buffer_size=50000, demo_buffer_size=1e4,
        ...
        self.train_curiosity_freq = 500
        self.curiosity_gradient_steps = 10
        self.self_learning_starts = self_learning_starts 
        self.expert_scores = []
        self.config = config
    ...
    def setup_model(self, config={}):
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                self.onpolicy = 'onpolicy' in config['shaping_mode']
                self.pi_reg_coeff = config.get('pi_reg_coeff', 1e-2)
                self.lfd = 'lfd' in config['shaping_mode'] 
                self.mix = self.lfd
                self.curiosity = 'curiosity' in config['shaping_mode']

                self.replay_buffer = ReplayBuffer(self.buffer_size)
                self.teacher_buffer = ReplayBuffer(self.demo_buffer_size)
                self.episode_buffer = TrajectoryBuffer(self.buffer_size) # cache the most recent episode visited by our policy 
                self.cache_buffer = TrajectoryBuffer(2048) # cache the most recent episode visited by our policy 
                ...
                with tf.variable_scope("model", reuse=False):
                    ...
                    qf1_pi, _ = self.policy_tf.make_critics(self.processed_obs_ph, policy_out, reuse=True)
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    policy_loss = self.policy_loss = -tf.reduce_mean(qf1_pi)
    ...
    def get_imitate_reward(self, batch_obs, batch_actions, batch_next_obs, batch_rewards, batch_dones):
        mode = self.config['shaping_mode']
        if mode == 'train':
            return batch_rewards
        rewards = self.discriminator.get_reward(batch_obs, batch_actions, unscale=False, sess=self.sess)
        if self.curiosity:
            curiosity_rewards = 0.01 * self.explorer.get_reward(batch_obs, batch_actions, batch_next_obs, unscale=False, sess=self.sess)
            curiosity_rewards = np.clip(curiosity_rewards, 0, 1) 
            rewards = rewards + curiosity_rewards 
        return rewards
    ...
    def generate_train_data(self, step, mode=None): 
        ...
        if (self.lfd and self.mix ): # sample both self-data and expert data
            expert_batch = self.teacher_buffer.sample(expert_batch_size)
            self_batch = self.replay_buffer.sample(self_batch_size)
            #print(expert_batch[2].shape, self_batch[2].shape)
            #batch_obs, batch_actions, batch_true_rewards, batch_next_obs, batch_dones = batch
    ...
    def _train_step(self, step, writer, learning_rate, update_policy):
        ...
        rewards = self.get_imitate_reward(batch_obs, batch_actions, batch_next_obs, batch_rewards, batch_dones)
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for step in range(total_timesteps):
                ...
                if self.num_timesteps < self.learning_starts or np.random.rand() < self.random_exploration:
                    ...
                    action = rescaled_action = self.env.action_space.sample()
                else:
                ...
                # Store transition in the caching buffer
                self.episode_buffer.add(obs, action, reward, new_obs, float(done))
                self.cache_buffer.add(obs, action, reward, new_obs, float(done))
                obs = new_obs
                ...
                # update caching buffer and/or teacher buffer once we collect one complete episode
                if self.adaptive and done and episode_score is not None:
                    if episode_score > self.expert_scores[0]:
                        self.mix = False # once we reach threshold, we should stop using teacher data
                        # update lowerbound for teacher buffer 
                        self.expert_scores.pop(0)
                        self.expert_scores.append(episode_score)
                        self.expert_scores.sort()
                        print("Adding episode with score {} to teacher buffer".format(episode_score))
                        # copy transition in caching buffer to teacher buffer
                        for transition in self.episode_buffer.get_episode():
                            s, a, r, s_1, if_done = transition
                            self.teacher_buffer.add(s,a,r,s_1,if_done)
                    #else:
                        #print("Skipping episode with score {}/ {}".format(episode_score, self.expert_scores[0]))

                    self.episode_buffer.reset() # cleanup episode buffer
                ...
                if step % self.train_discriminator_freq == 0:
                    if  self.num_timesteps >= self.learning_starts and self.teacher_buffer.can_sample(self.batch_size): 
                        ...
                        self.discriminator.train_discriminator(
                            ...
                            current_lr,
                            self.teacher_buffer,
                            self.cache_buffer if self.onpolicy else self.replay_buffer, # [\dpi or \dB depends on which version of algorithm we use]
                            self.batch_size,
                            self.sess) 

                if step % self.train_curiosity_freq == 0:
                    if  self.num_timesteps >= self.learning_starts and self.teacher_buffer.can_sample(self.batch_size): 
                        if self.curiosity: 
                            self.explorer.train_classifier(
                                logger,
                                step,
                                self.curiosity_gradient_steps,
                                current_lr,
                                self.replay_buffer,
                                self.batch_size,
                                self.sess)
