...
class TD3DACFO(OffPolicyRLModel):
    """
    DAC from observations (DACfO).
    Difference from DAC: use only states (s,s') to train the discriminator.
    ...
    def __init__(self, policy, env, gamma=0.99, learning_rate=3e-4, buffer_size=50000, demo_buffer_size=1e4,
                 ...
                 absorbing_per_episode=5,
                 config={}):

        super(TD3DACFO, self).__init__(policy=policy, env=env, replay_buffer=None, verbose=verbose,
    ...
    def initialize_teacher_buffer(self):
        data = self.expert_dataset.get_transitions()
        # add extra dimension to indicate whether the state is aborbing 
        demo_obs, demo_actions, demo_rewards, demo_dones, demo_next_obs, demo_episode_scores = self.expert_dataset.add_absorbing_states(data, self.env)
    ...
    def setup_model(self, config):
        ...
        self.max_episode_steps = self.env.envs[0].max_episode_steps
    ...
    def learn(self, total_timesteps, callback=None,
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for step in range(total_timesteps):
                ...
                ##############################################
                truncated_done = done and episode_timesteps + 1 == self.max_episode_steps  # pylint: disable=protected-access

                if done:
                    if not truncated_done:
                        #print("Step {} /{}, Change to absorbing states ....".format(episode_timesteps+1, self.max_episode_steps))
                        new_obs = self.env.envs[0].get_absorbing_state()
                    else:
                        print("Step {} /{}. Episode ends due to max-episode-step reached.".format(episode_timesteps+1, self.max_episode_steps))
                ############################################## 
                # Add several absobrsing states to absorbing states transitions.
                ############################################## 
                if done and not truncated_done:
                    for abs_i in range(self.absorbing_per_episode):
                        if abs_i + episode_timesteps < self.max_episode_steps:  # pylint: disable=protected-access
                            obs = self.env.envs[0].get_absorbing_state()
                            action = self.env.action_space.sample()
                            next_obs = self.env.envs[0].get_absorbing_state() 
                            #self.episode_buffer.add(obs, action, 0, next_obs, 1.0, 0, episode_score, 0)
                            self.replay_buffer.add(obs, action, 0, new_obs, 1.0 )
