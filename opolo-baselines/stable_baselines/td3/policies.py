...
class TD3Policy(BasePolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env=1, n_steps=1, n_batch=None, reuse=False, scale=False, OffPolicyRLModel=False):
...
class FeedForwardPolicy(TD3Policy):
    ...
    def make_actor(self, obs=None, reuse=False, scope="pi"):
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            policy = tf.layers.dense(pi_h, self.ac_space.shape[0], activation=tf.tanh)
            if not reuse:
                self.policy = policy
