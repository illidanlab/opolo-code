...
from stable_baselines.deepq.replay_buffer import ReplayBuffer, NStepPrioritizedReplayBuffer, NStepReplayBuffer
...
class DDPGfD(OffPolicyRLModel):
    ...
    def __init__(self, policy, env, gamma=0.99, memory_policy=None, eval_env=None, nb_train_steps=50,
        ...
        super(DDPGfD, self).__init__(policy=policy, env=env, replay_buffer=None,
        ...
        self.prioritized_replay_alpha = 0.3
        self.prioritized_replay_beta = 1
        self.use_n_step = True
        self.n_step = 5
        self.nstep_lambda = 0.1
        self.actor_loss_lambda = 0.1
        self.eps = 1e-2
        self.d_eps = 1e-1
        self.sparse_rewards = 'sparse' in config['shaping_mode'] 
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("input", reuse=False):
                    ...
                    self.policy_tf = self.policy(self.sess, self.observation_space, self.action_space, 1, 1, None, **self.policy_kwargs)

                    # Create target networks.
                    self.target_policy = self.policy(self.sess, self.observation_space, self.action_space, 1, 1, None, **self.policy_kwargs)
                    self.obs_target = self.target_policy.obs_ph # next-obs
                    self.action_target = self.target_policy.action_ph

                    self.n_next_obs =  tf.placeholder(tf.float32, (None,) + self.observation_space.shape, name="n_step_next_observations_ph")
                    #self.n_action_target = None
                    self.n_rewards = tf.placeholder(tf.float32, shape=(None, 1), name='n_step_rewards')

                    self.n_critic_target = tf.placeholder(tf.float32, shape=(None, 1), name='n_critic_target')
                    ...
                    normalized_next_obs = tf.clip_by_value(normalize(self.target_policy.processed_obs, self.obs_rms),
                                                       self.observation_range[0], self.observation_range[1])
                    normalized_n_next_obs = tf.clip_by_value(normalize(self.n_next_obs, self.obs_rms),
                                                       self.observation_range[0], self.observation_range[1])
                    ...
                    self.critic_n_target = tf.placeholder(tf.float32, shape=(None, 1), name='critic_n_target')
                ...
                with tf.variable_scope("model", reuse=False):
                    ...
                    self.normalized_critic_with_actor_tf = self.policy_tf.make_critic(normalized_obs, self.actor_tf, reuse=True)
                ...
                with tf.variable_scope("target", reuse=False):
                    ...
                    critic_n_target = self.target_policy.make_critic(normalized_n_next_obs,
                                                                   self.target_policy.make_actor(normalized_n_next_obs, reuse=True), reuse=True)
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    q_n_next_obs = denormalize(critic_n_target, self.ret_rms)

                    self.target_q = self.rewards + (1. - self.terminals_ph) * self.gamma * q_next_obs
                    self.target_n_q = self.n_rewards + (1. - self.terminals_ph) * self.gamma * q_n_next_obs

                    tf.summary.scalar('critic_target', tf.reduce_mean(self.critic_target))
                    tf.summary.scalar('critic_n_target', tf.reduce_mean(self.critic_n_target))
    ...
    def _setup_actor_optimizer(self):
        ...
        self.actor_loss_element = - self.critic_with_actor_tf
    ...
    def _setup_critic_optimizer(self):
        ...
        normalized_n_critic_target_tf = tf.clip_by_value(normalize(self.critic_n_target, self.ret_rms), self.return_range[0], self.return_range[1])

        critic_1_loss_element = tf.square(self.normalized_critic_tf - normalized_critic_target_tf)
        critic_1_loss = tf.reduce_mean(critic_1_loss_element)
        critic_n_loss_element = tf.square(self.normalized_critic_tf - normalized_n_critic_target_tf)
        critic_n_loss = tf.reduce_mean(critic_n_loss_element)
        self.critic_loss = critic_1_loss + self.nstep_lambda * critic_n_loss
        self.critic_loss_element = critic_1_loss_element + self.nstep_lambda * critic_n_loss_element
    ...
    def initialize_buffers(self):
        ...
        demo_size = n_samples + 1 - self.n_step
        # initialize both buffers
        self.n_step_replay_buffer = NStepReplayBuffer(self.buffer_size, self.n_step, demo_size, self.gamma)
        self.replay_buffer = NStepPrioritizedReplayBuffer(self.buffer_size, self.prioritized_replay_alpha, self.n_step, self.gamma, demo_size) 
        episode_reward = 0
        for i in range(n_samples):
            s, a, r, ns, done = demo_obs[i], demo_actions[i], demo_rewards[i], demo_next_obs[i], float(demo_dones[i])
            a = scale_action(self.action_space, a)
            r *= self.reward_scale
            episode_reward += r 
            if self.sparse_rewards:
                shaped_reward = 0.01 * float(done) * episode_reward
            else:
                shaped_reward = r
            self._store_transition(s, a, shaped_reward, ns, done, demo=True)
            if done:
                episode_reward = 0

        self.n_step_replay_buffer.queue.clear()
        #print(len(self.n_step_replay_buffer), len(self.replay_buffer), n_samples, demo_size, self.n_step)




    def _store_transition(self, obs, action, reward, next_obs, done, demo=False):
        """
        ...
        transition = self.n_step_replay_buffer.add_transition(obs, action, reward, next_obs, done, demo=demo)
        if transition is not None: # n-step is not ready 
            observation, action, reward, next_observation, done =  transition
            # add it to prioritized replay buffer 
            self.replay_buffer.add(observation, action, reward, next_observation, float(done), demo=demo)
            if self.normalize_observations and self.obs_rms:
    ...
    def _train_step(self, step, writer, log=False):
        ...
        """
        # Get a batch
        obs, actions, rewards, next_obs, terminals, if_demo, weights, idxes = self.replay_buffer.sample(batch_size=self.batch_size, beta=self.prioritized_replay_beta)
        obs_n, act_n, reward_n, next_obs_n, _, _ = self.n_step_replay_buffer.sample_by_index(idxes)
        # Reshape to match previous behavior and placeholder shape
        rewards = rewards.reshape(-1, 1)
        terminals = terminals.reshape(-1, 1)
        reward_n = reward_n.reshape(-1, 1)
        if_demo = if_demo.reshape(-1,1)
        #next_obs = next_obs.reshape(-1, self.observation_space.shape[0])
        #next_obs_n = next_obs_n.reshape(-1, self.observation_space.shape[0])
        #print(reward_n.shape, rewards.shape, np.mean(reward_n), np.mean(rewards))
        #done_n = done_n.reshape(-1,1)
        

        if self.normalize_returns and self.enable_popart:
            old_mean, old_std, target_q, target_n_q = self.sess.run([self.ret_rms.mean, self.ret_rms.std, self.target_q, self.target_n_q],
                                                        feed_dict={
                                                            self.obs_target: next_obs,
                                                            self.rewards: rewards,
                                                            self.terminals_ph: terminals,
                                                            self.n_next_obs: next_obs_n,
                                                            self.n_rewards: reward_n 
                                                        })
            ...
            target_q, target_n_q = self.sess.run([self.target_q, self.target_n_q], feed_dict={
                self.obs_target: next_obs,
                self.rewards: rewards,
                self.terminals_ph: terminals,
                self.n_rewards: reward_n,  
        ...
        ops = [self.actor_grads, self.actor_loss, self.critic_grads, self.critic_loss, self.actor_loss_element, self.critic_loss_element]
        td_map = {
            self.obs_train: obs,
            ...
            self.critic_n_target: target_n_q, 
        ...
        if writer is not None:
            ...
            if self.full_tensorboard_log and log and step not in self.tb_seen_steps:
                ...
                summary, actor_grads, actor_loss, critic_grads, critic_loss, actor_loss_element, critic_loss_element = \
                ...
                summary, actor_grads, actor_loss, critic_grads, critic_loss, actor_loss_element, critic_loss_element = self.sess.run([self.summary] + ops,
                                                                                            td_map)
            writer.add_summary(summary, step)
        else:
            actor_grads, actor_loss, critic_grads, critic_loss, actor_loss_element, critic_loss_element  = self.sess.run(ops, td_map)
        ...
        # update priorities
        new_priorities = critic_loss_element + self.actor_loss_lambda * np.abs(actor_loss_element) + self.eps + if_demo * self.d_eps
        #print(new_priorities.shape, idxes.shape)
    ...
    def _get_stats(self):
        ...
        if self.stats_sample is None:
            ...
            obs, actions, rewards, next_obs, terminals, if_demos, weights, idxes = self.replay_buffer.sample(batch_size=self.batch_size, beta=self.prioritized_replay_beta)
            self.stats_sample = {
                'obs': obs,
                'actions': actions,
                'rewards': rewards,
    ...
    def _adapt_param_noise(self):
        ...
        obs, *_ = self.replay_buffer.sample(batch_size=self.batch_size, beta=self.prioritized_replay_beta)
                ...
                while True:
                    for _ in range(log_interval):
                        ...
                        for _ in range(self.nb_rollout_steps):
                            ...
                            if self.sparse_rewards:
                                shaped_reward = self.reward_scale * float(done) * episode_reward * 0.01 
                            else:
                                shaped_reward = reward
                            self._store_transition(obs, action, shaped_reward, new_obs, done)
                            obs = new_obs
