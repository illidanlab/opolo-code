...
def total_reward_logger(
    rew_acc, gail_rew_acc, entropy_rew_acc, mi_rew_acc,
    rewards, gail_rewards, entropy_rewards, mi_rewards,
    masks, writer, steps):
    """
    calculates the cumulated episode reward, and prints to tensorflow log the output
    ...
    with tf.variable_scope("environment_info", reuse=True):
        for env_idx in range(rewards.shape[0]):
            ...
            if len(dones_idx) == 0:
                rew_acc[env_idx] += sum(rewards[env_idx])
                gail_rew_acc[env_idx] += sum(gail_rewards[env_idx])
                entropy_rew_acc[env_idx] += sum(entropy_rewards[env_idx])
                mi_rew_acc[env_idx] += sum(mi_rewards[env_idx])
            else:
                rew_acc[env_idx] += sum(rewards[env_idx, :dones_idx[0, 0]])
                gail_rew_acc[env_idx] += sum(gail_rewards[env_idx, :dones_idx[0, 0]])
                mi_rew_acc[env_idx] += sum(mi_rewards[env_idx, :dones_idx[0, 0]])
                entropy_rew_acc[env_idx] += sum(entropy_rewards[env_idx, :dones_idx[0, 0]])

                summary = tf.Summary(value=[tf.Summary.Value(tag="episode_reward", simple_value=rew_acc[env_idx])])
                gail_summary = tf.Summary(value=[tf.Summary.Value(tag="gail_reward", simple_value=gail_rew_acc[env_idx])])
                mi_summary = tf.Summary(value=[tf.Summary.Value(tag="mi_reward", simple_value=mi_rew_acc[env_idx])])
                entropy_summary = tf.Summary(value=[tf.Summary.Value(tag="entropy_reward", simple_value=entropy_rew_acc[env_idx])])
                writer.add_summary(summary, steps + dones_idx[0, 0])
                writer.add_summary(gail_summary, steps + dones_idx[0, 0])
                writer.add_summary(mi_summary, steps + dones_idx[0, 0])
                writer.add_summary(entropy_summary, steps + dones_idx[0, 0])
                for k in range(1, len(dones_idx[:, 0])):
                    rew_acc[env_idx] = sum(rewards[env_idx, dones_idx[k-1, 0]:dones_idx[k, 0]])
                    summary = tf.Summary(value=[tf.Summary.Value(tag="episode_reward", simple_value=rew_acc[env_idx])])
                    writer.add_summary(summary, steps + dones_idx[k, 0])
